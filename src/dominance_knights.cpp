...
[[nodiscard]] static std::vector<std::vector<uint8_t>> build_cycle_dominance(uint32_t players) {
    std::vector<std::vector<uint8_t>> m(players, std::vector<uint8_t>(players, 0));
    for (uint32_t i = 0; i < players; ++i) {
        const uint32_t j = (i + 1U) % players;
        m[i][j] = 1; // i blocks j
    }
    return m;
}

[[nodiscard]] static std::vector<std::vector<uint8_t>> build_none_dominance(uint32_t players) {
    return std::vector<std::vector<uint8_t>>(players, std::vector<uint8_t>(players, 0));
}

[[nodiscard]] static std::vector<std::vector<uint8_t>> build_custom_dominance(uint32_t players, const std::string& edges_text) {
    auto m = build_none_dominance(players);
    if (edges_text.empty()) return m;

    std::stringstream ss(edges_text);
    std::string token;
    while (std::getline(ss, token, ',')) {
        token.erase(std::remove_if(token.begin(), token.end(), ::isspace), token.end());
        if (token.empty()) continue;
        const size_t arrow = token.find('>');
        if (arrow == std::string::npos) throw std::runtime_error("invalid edge token: " + token);

        const uint32_t from = static_cast<uint32_t>(std::stoul(token.substr(0, arrow)));
        const uint32_t to = static_cast<uint32_t>(std::stoul(token.substr(arrow + 1)));
        if (from == 0 || to == 0 || from > players || to > players) {
            throw std::runtime_error("edge out of range in: " + token + " (indices are 1..players)");
        }
        m[from - 1U][to - 1U] = 1;
    }
    return m;
...
[[nodiscard]] static bool is_legal(
    ...
    const std::vector<std::vector<uint8_t>>& dominance
) {
    if (occupied.find(target) != occupied.end()) return false;

    for (uint32_t attacker = 0; attacker < attack_maps.size(); ++attacker) {
        if (dominance[attacker][player] == 0) continue;
        const auto it = attack_maps[attacker].find(target);
        if (it != attack_maps[attacker].end() && it->second > 0) return false;
    }
    return true;
}
...
[[nodiscard]] static SimResult simulate(
    uint32_t players,
    uint64_t moves,
    const std::vector<std::vector<uint8_t>>& dominance,
    const std::string& mode,
    const std::string& edges_text
) {
    if (players < 2) throw std::runtime_error("players must be >= 2");

    SimResult result;
    result.state.rules = "dominance_knights";
    result.state.players = players;
    result.state.moves_requested = moves;
    result.state.moves_completed = 0;
    result.state.dominance_mode = mode;
    result.state.dominance_edges = edges_text;
    ...
    std::vector<std::unordered_map<Coord, uint32_t, CoordHash>> attack_maps(players);
    ...
    for (uint64_t turn = 0; turn < moves; ++turn) {
        ...
        while (true) {
            ...
            if (is_legal(p, c, occupied, attack_maps, dominance)) {
                occupied.insert(c);
                result.state.placements.push_back(Placement{n, c, p});
                add_attacks(c, attack_maps[p]);
                cursors[p] = n + 1ULL;
...
static void save_state_json(const GameState& state, const std::string& out_path) {
    ...
    out << "  \"dominance_mode\": \"" << state.dominance_mode << "\",\n";
    out << "  \"dominance_edges\": \"" << state.dominance_edges << "\",\n";
    out << "  \"placements\": [\n";
...
[[nodiscard]] static std::string extract_string(const std::string& txt, const std::string& key, const std::string& fallback) {
    const std::regex re("\\\"" + key + "\\\"\\s*:\\s*\\\"([^\\\"]*)\\\"");
    std::smatch m;
    if (std::regex_search(txt, m, re)) return m[1].str();
    return fallback;
}
...
[[nodiscard]] static GameState load_state_json(const std::string& path) {
    ...
    s.dominance_mode = extract_string(txt, "dominance_mode", "");
    s.dominance_edges = extract_string(txt, "dominance_edges", "");
...
[[nodiscard]] static std::vector<RGB> build_palette(uint32_t players) {
    ...
    const RGB seed[] = {
        {255, 0, 0}, {0, 0, 0}, {0, 180, 0}, {0, 90, 255}, {255, 220, 0}, {150, 0, 180}, {255, 120, 0}
    };
    for (uint32_t i = 0; i < players; ++i) {
        if (i < sizeof(seed) / sizeof(seed[0])) colors.push_back(seed[i]);
        else colors.push_back(hsv_to_rgb(std::fmod(i * 137.50776f, 360.0f), 0.85f, 0.95f));
    }
    return colors;
}
...
[[nodiscard]] static std::string default_state_path(uint32_t players, const std::string& mode, uint64_t moves) {
    return "outputs/dominance_" + mode + "_" + std::to_string(players) + "/states/state_" + std::to_string(moves) + ".json";
}

[[nodiscard]] static std::string default_render_path(uint32_t players, const std::string& mode, uint64_t moves, uint64_t cells, uint32_t cell_px) {
    return "outputs/dominance_" + mode + "_" + std::to_string(players) + "/renders/board_" + std::to_string(moves) + "_cells_" + std::to_string(cells) + "_px_" + std::to_string(cell_px) + ".png";
}

static void print_usage() {
    std::cerr
        << "Usage:\n"
        << "  dominance_knights simulate --players N --moves M --dominance cycle|none|custom [--edges \"1>2,2>3\"] [--out state.json]\n"
        << "  dominance_knights render --state state.json --xmin A --xmax B --ymin C --ymax D [--cell P] [--png out.png]\n"
        << "  dominance_knights simulate-render --players N --moves M --cells C --dominance cycle|none|custom [--edges \"1>2,2>3\"] [--cell P] [--state out.json] [--png out.png]\n";
}

[[nodiscard]] static std::vector<std::vector<uint8_t>> parse_dominance(
    uint32_t players,
    const std::string& mode,
    const std::string& edges
) {
    if (mode == "cycle") return build_cycle_dominance(players);
    if (mode == "none") return build_none_dominance(players);
    if (mode == "custom") return build_custom_dominance(players, edges);
    throw std::runtime_error("dominance must be one of: cycle, none, custom");
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (cmd == "simulate") {
            ...
            const std::string mode = required_arg(a, "--dominance");
            const std::string edges = (a.find("--edges") != a.end()) ? a.at("--edges") : "";
            const auto dominance = parse_dominance(players, mode, edges);

            const std::string out = (a.find("--out") != a.end()) ? a.at("--out") : default_state_path(players, mode, moves);
            SimResult r = simulate(players, moves, dominance, mode, edges);
        ...
        if (cmd == "render") {
            ...
            const std::string mode = s.dominance_mode.empty() ? "unknown" : s.dominance_mode;
            const std::string png = (a.find("--png") != a.end()) ? a.at("--png") : default_render_path(s.players, mode, s.moves_completed, cells, cell_px);
        ...
        if (cmd == "simulate-render") {
            ...
            const std::string state_out = (a.find("--state") != a.end()) ? a.at("--state") : default_state_path(players, mode, moves);
            const std::string png_out = (a.find("--png") != a.end()) ? a.at("--png") : default_render_path(players, mode, moves, side * side, cell_px);
